...
impl MySqlUpdater {
    ...
    pub fn run<T>(&mut self,  event_handler: T) -> Result<(), String> where T:MySqlUpdaterEvents + Sized + Send + Sync{
        ...
        while !self.shutdown.load(Ordering::SeqCst) {
            for table_name in self.config.tables.keys() {
                debug!("get_count_query: db_name:{}, table_name:{}", &self.config.db_config.db_name, &table_name);
                let sql_count_query = event_handler.get_count_query(&self.config.db_config.db_name, &table_name);
                ...
                event_handler.total_rows_count(&self.config.db_config.db_name, &table_name, total_count);
                ...
                for _i in 0..iterations {
                    ...
                    let fetch_query = event_handler.get_fetch_query(&self.config.db_config.db_name, &table_name, offset, self.config.fetch_limit);
                    ...
                    if res.is_err() {
                        ...
                        event_handler.rows_fetched(&self.config.db_config.db_name, &table_name, offset, rows);
                    }
            ...
            if !self.shutdown.load(Ordering::SeqCst) {
                std::thread::sleep(Duration::from_millis(self.config.periodic_fetch_duration.into()));
            }
        }
        Ok(())
    }
